...
func newUserF5Controller(ctx context.Context, clusterContext *config.UserContext) *UserF5Controller {
	n := &UserF5Controller{
...
func (ic *UserF5Controller) reconcileAllGlobalDNSs() (runtime.Object, error) {
...
func (ic *UserF5Controller) doesGlobalDNSTargetCurrentCluster(globalDNS *v3.GlobalDNS) (bool, error) {
	...
	if globalDNS.Spec.MultiClusterAppName != "" {
		...
		if err != nil && k8serrors.IsNotFound(err) {
			logrus.Debugf("UserF5Controller: reconcileallgdns, multiclusterapp not found by name %v, might be marked for deletion", mcappName)
	...
	for _, projectNameSet := range targetProjectNames {
		...
		if len(split) != 2 {
			return false, fmt.Errorf("UserF5Controller: Error in splitting project Name %v", projectNameSet)
		}
