...
while IFS= read -r i; do
    ...
    if [[ $from_aliyun == "true" ]]; then
        ...
        if docker pull "$aliyun_registry/$i" > /dev/null 2>&1 ; then
            echo "Image pull success from Ali Cloud: ${i}"
            docker tag "$aliyun_registry/$i" "$i"
            docker rmi "$aliyun_registry/$i" > /dev/null 2>&1
            pulled="${pulled} ${i}"
        elif docker pull "${i}" > /dev/null 2>&1; then
            echo "Image pull success from DockerHub: ${i}"
            pulled="${pulled} ${i}"
        else
